...
(() => {
  ...
  // can opt into a bigger cap with the `data-store-width` attribute.
  const MAX_DIM = 2200;
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      const gen = ++this._gen;
      try {
        const attrW = parseInt(this.getAttribute('data-store-width'), 10);
        const w = Number.isFinite(attrW) && attrW > 0 ?
          attrW : this.clientWidth || this.offsetWidth || MAX_DIM;
